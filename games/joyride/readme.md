# Joyride

> Verified game note.

![Joyride screenshot placeholder](joyride-placeholder.svg)

## At a glance

- **Score:** 8.0/10
- **Model:** Claude Fable 5.1
- **Technology:** Three.js, JavaScript, Web Audio API
- **Verified:** 2026-09-09
- **Repository:** [https://github.com/nitzangames/joyride](https://github.com/nitzangames/joyride)
- **Evidence:** [direct model evidence](https://github.com/nitzangames/joyride)
- **Live demo:** [open demo](https://nitzan.games/experiments/one-shot-gta/fable-xhigh/index.html)
- **Additional live link:** [open demo](https://nitzan.games/OneShotGTA.html)

## Screenshots

No screenshot source is recorded yet. Keep the placeholder until a repository, awesome list, or creator source provides an image.

## Model attribution

Open the evidence link above. Evidence grade: **direct model evidence**.

## Source description

Playable open-world driving sandbox with vehicle theft, traffic, crash loop, physics, controls, capture harness, and explicit Fable 5.1 build record.

### Gameplay source

- [https://github.com/nitzangames/joyride](https://github.com/nitzangames/joyride)

## Verification notes

- **Status:** verified
- **Counted units:** 1
- **Discovery:** https://github.com/nitzangames/joyride

[Back to the awesome list](../../README.md)
